defmodule DiscordCloneWeb.VideoCallLive.TwilioAccessToken do
  @moduledoc """
  A Twilio JWT access token, as described in the Twilio docs.

  https://www.twilio.com/docs/iam/access-tokens
  """
   alias DiscordCloneWeb.VideoCallLive.Grant
  use Joken.Config
  ...
  def to_jwt!(token) do
    token =
      token
      |> validate!(:account_sid, &is_binary/1, "must be a binary")
      |> validate!(:api_key, &is_binary/1, "must be a binary")
      |> validate!(:api_secret, &is_binary/1, "must be a binary")
      |> validate!(:identity, &is_binary/1, "must be a binary")
      |> validate!(:grants, &list_of_grants?/1, "must be a list of grants")
      |> validate!(:expires_in, &is_integer/1, "must be an integer")

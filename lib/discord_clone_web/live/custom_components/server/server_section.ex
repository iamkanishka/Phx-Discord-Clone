defmodule DiscordCloneWeb.CustomComponents.Server.ServerSection do
  ...
  def render(assigns) do
    ...
    <div class="flex items-center justify-between py-2">
      ...
      <%= if role != "GUEST" and @sectionType == "channels" %>

        <ActionTooltip label="Create Channel" side="top">
          <button
          phx-click="create_channel" phx-target={@myself}
     ...
     <% end %>
     <%= if role != "ADMIN" and @sectionType == "members" %>
      <%!-- <ActionTooltip label="Manage Members" side="top"> --%>
          <button phx-click="members" phx-target={@myself} class="text-zinc-500 hover:text-zinc-600 dark:text-zinc-400 dark:hover:text-zinc-300 transition">
